package com.atgenomix.seqslab.tasks.operator
...
import com.atgenomix.seqslab.operators.transformer.NormalizeSchemaTransformerFactory
import com.atgenomix.seqslab.{SparkHadoopSessionBuilder, TestUtil}
...
class TransformerSpec extends AnyFlatSpec with SparkHadoopSessionBuilder {
  ...
  it should "convert special charactor into underscore" in {
    ...
    val operatorPluginMap = Map("NormalizeSchemaTransformer" -> "com.atgenomix.seqslab.TestPlugin")
    val piperCtx = new SeqslabPiperContext(_spark, SeqslabPipeline(Map.empty, Map.empty, Map.empty, Map.empty), None)
    val pluginContextMap = Map("com.atgenomix.seqslab.TestPlugin" ->  new PluginContext(piperCtx))
