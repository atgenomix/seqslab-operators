...
import com.atgenomix.seqslab.piper.plugin.atgenomix.operators.partitioner.BamPartitionerHg19Part155Factory.BamPartitionerHg19Part155
...
object BamPartitionerHg19Part155Factory {
  class BamPartitionerHg19Part155(pluginCtx: PluginContext, operatorCtx: OperatorContext) extends Transformer {
    ...
    override def call(t1: Dataset[Row]): Dataset[Row] = {
      val bed = getClass.getResource("/bed/19/contiguous_unmasked_regions_155_parts")
      val dict = getClass.getResource("/reference/19/GRCH/ref.dict")
      val partitioner = GenomicPartitioner(Array(bed), dict)

      val keyColumn = functions.call_udf("hg19part155", t1.col("referenceName"), t1.col("alignmentStart"))
      val result = t1
...
class BamPartitionerHg19Part155Factory extends TransformerSupport {
  override def createTransformer(pluginContext: PluginContext, operatorContext: OperatorContext): Transformer = {
    new BamPartitionerHg19Part155(pluginContext, operatorContext)
  }
}
