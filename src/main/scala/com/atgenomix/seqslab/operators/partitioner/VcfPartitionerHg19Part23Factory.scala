...
import com.atgenomix.seqslab.piper.plugin.atgenomix.operators.partitioner.VcfPartitionerHg19Part23Factory.VcfPartitionerHg19Part23
...
object VcfPartitionerHg19Part23Factory {
  class VcfPartitionerHg19Part23(pluginCtx: PluginContext, operatorCtx: OperatorContext) extends Transformer {
    ...
    override def call(t1: Dataset[Row]): Dataset[Row] = {
      ...
      val parallelism = getClass.getResource("/bed/19/chromosomes")
      val ref = getClass.getResource("/reference/19/GRCH/ref.dict")
      val key_column = functions.call_udf("hg19part23", t1.col("row"), lit(Int.MinValue))
...
class VcfPartitionerHg19Part23Factory extends TransformerSupport {
  override def createTransformer(pluginContext: PluginContext, operatorContext: OperatorContext): Transformer = {
    new VcfPartitionerHg19Part23(pluginContext, operatorContext)
  }
}
