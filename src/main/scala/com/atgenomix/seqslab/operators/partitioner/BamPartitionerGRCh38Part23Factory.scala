...
import com.atgenomix.seqslab.piper.plugin.atgenomix.operators.partitioner.BamPartitionerGRCh38Part23Factory.BamPartitionerGRCh38Part23
...
object BamPartitionerGRCh38Part23Factory {
  class BamPartitionerGRCh38Part23(pluginCtx: PluginContext, operatorCtx: OperatorContext) extends Transformer {
    ...
    override def call(t1: Dataset[Row]): Dataset[Row] = {
      val bed = getClass.getResource("/bed/38/chromosomes")
      val dict = getClass.getResource("/reference/38/GRCH/ref.dict")
      val partitioner = GenomicPartitioner(Array(bed), dict)

      val keyColumn = functions.call_udf("grch38part23", t1.col("referenceName"), t1.col("alignmentStart"))
      val result = t1
...
class BamPartitionerGRCh38Part23Factory extends TransformerSupport {
  override def createTransformer(pluginContext: PluginContext, operatorContext: OperatorContext): Transformer = {
    new BamPartitionerGRCh38Part23(pluginContext, operatorContext)
  }
}
