...
object VcfPartitionerFactory {
  class VcfPartitioner(pluginCtx: PluginContext, operatorCtx: OperatorContext) extends Transformer with SupportsPartitioner {
    private var refSeqDict: java.net.URL = null
    private var partBed: java.net.URL = null
    private val opName: String = "VcfPartitioner"
    private var udfName: String = null

    override def init(i: Int, i1: Int): Transformer = {
      val ref = operatorCtx.get("VcfPartitioner:refSeqDict")
      val bed = operatorCtx.get("VcfPartitioner:partBed")
      ...
      } else throw new IllegalArgumentException("VcfPartitioner:refSeqDict cannot be null")
      ...
      } else throw new IllegalArgumentException("VcfPartitioner:partBed cannot be null")
...
class VcfPartitionerFactory extends TransformerSupport {
  override def createTransformer(pluginContext: PluginContext, operatorContext: OperatorContext): Transformer = {
    new VcfPartitioner(pluginContext, operatorContext)
  }
}
