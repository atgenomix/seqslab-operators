...
class BgenFileFormat extends FileFormat with DataSourceRegister with Serializable with GlowLogging {
  ...
  override def shortName(): String = "smallbgen"

  override def inferSchema(sparkSession: SparkSession,
                           options: Map[String, String],
                           files: Seq[FileStatus]): Option[StructType] = {
    Option(StructType(Seq(
      BgenFileFormat.contigNameField,
      startField,
      endField,
      namesField,
      refAlleleField,
      alternateAllelesField,
      StructField("genotypes", BinaryType)
    )))
  }
  ...
  override def buildReader(spark: SparkSession,
   ...
   */
  private def nextVariantIndex(hadoopFs: FileSystem,
  ...
  private def downloadIdxIfNecessary(hadoopFs: FileSystem, path: Path): String = {
    ...
    WithUtils.withLock(BgenFileFormat.idxLock.get(path)) {
      ...
      val fileStatus = hadoopFs.listStatus(path)
      val remoteSize = fileStatus.head.getLen
      var localSize = 0L
      // Wait until index is fully downloaded.
      do {
        localSize = Paths.get(localPath).toFile.length()
        Thread.sleep(2000)
...
object BgenFileFormat extends HlsEventRecorder {
  ...
  val contigNameField: StructField = StructField("contigName", IntegerType)
  ...
  def getBgenSampleIds(options: Map[String, String], hadoopConf: Configuration, path: Path): Option[Array[String]] = {
    ...
    new BgenHeaderReader(littleEndianStream).readHeader(sampleIdsOpt).sampleIds
