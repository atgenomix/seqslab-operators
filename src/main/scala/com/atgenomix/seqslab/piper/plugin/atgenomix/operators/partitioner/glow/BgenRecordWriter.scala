 ...
 */
class BgenRecordWriter(outputStream: OutputStream,
                       schema: StructType,
                       writeHeader: Boolean,
                       numVariants: Long,
                       bitsPerProb: Int,
  ...
  private def writeVariantIdentifyingBlock(row: BgenRow): Unit = {
    ...
    writeUTF8String(row.contigName.toString, false, stream)
  ...
  def getCompressedGenotypeProbabilityData(row: BgenRow): (Int, Array[Byte]) = {
    ...
    ledos.write(row.genotypes.toArray)
  ...
  def write(row: InternalRow): Unit = {
    ...
    maybeWriteHeader(sampleIds.get.map(Option(_)))
